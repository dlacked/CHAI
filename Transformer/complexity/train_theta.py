...
def main():
    ...
    model_dir_default = COMPLEXITY_DIR / "model_theta"
    runs_dir_default = COMPLEXITY_DIR / "runs_theta"

    # NOTE (2026-08-31): train.py (the main model) is now POOLED (both jaws, one model) on top
    # of its earlier no-theta/no-PCA/no-X-mirror coordinate swap - this script stays per-jaw
    # (--jaw lower/upper, two separate models) on the OLD PCA-rotated coordinates. So this is no
    # longer a controlled "theta only" ablation in any sense - theta, coordinate scheme, AND
    # pooling all differ now. Treat it as "earliest design point" vs. "final design," not a
    # single-variable comparison, unless someone builds a properly isolated re-run.
    parser = argparse.ArgumentParser(description="Train the arch-level Transformer complexity classifier's theta ablation baseline (with theta, geom_dim=5, per-jaw, PCA-rotated coordinates - see the NOTE above for why this is no longer a clean ablation against train.py).")
