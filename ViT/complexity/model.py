...
class ArchComplexityTransformer(nn.Module):
    ...
    sequence - each token is just [x1,y1,x2,y2,theta], so the arch's tooth layout
    (position, spacing, angle) alone drives the prediction. No ResNet features (image
    embedding or per-tooth FDI confidence) are used - complexity is a property of the
    arch's geometric layout, not of what any individual tooth looks like or is classified
    as.
