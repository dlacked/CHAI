"""
Trains the Mirrored Variant (paper ablation label; formerly the main "Baseline"/"CHAI (Ours)"
model before CHAI was redefined to the unified 12-way, unmirrored architecture - see
train_chai.py, now the official model). Pooled (both-jaws, one model), no-rotation, no-theta
design (see functions/features/main_mirrored.py's docstring for the full reasoning): trains ONE
combined 6-way (digit-only) classifier on both jaws together, using
functions/features/main_mirrored.py's coordinates (ResNet/tooth/csv_mirrored/features_{train,val}.csv).
Retained as a trained ablation checkpoint (Table "Architecture-Component Ablation" in the paper) -
do not delete or retrain over this without updating that table.
...
Usage:
    .venv/Scripts/python.exe ResNet/tooth/train_mirrored.py [--resume]
"""
import sys
import os
import json
...
class ToothPositionClassifierNoTheta(ToothPositionClassifier):
    ...
    dropped from this ablation's features entirely (see main_mirrored.py's docstring: it's
...
class ToothDatasetCombined(Dataset):
    ...
    no theta) to match main_mirrored.py's CSV columns."""
...
def main():
    ...
    model_dir = project_root / "ResNet" / "tooth" / "model_mirrored"
    runs_dir = project_root / "ResNet" / "tooth" / "runs_mirrored"

    parser = argparse.ArgumentParser(description="Train the Mirrored Variant: a combined (both jaws, no-rotation, no-theta, unified mirror) 6-way tooth digit classifier.")
    ...
    parser.add_argument("--csv_dir", type=str, default=str(csv_dir),
                        help="Directory holding features_{train,val}.csv (default: ResNet/tooth/csv_mirrored)")
    parser.add_argument("--model_dir", type=str, default=str(model_dir),
                        help="Directory to save best.pth into (default: ResNet/tooth/model_mirrored)")
    parser.add_argument("--runs_dir", type=str, default=str(runs_dir),
                        help="Directory to save metric plots/confusion matrix into (default: ResNet/tooth/runs_mirrored)")
