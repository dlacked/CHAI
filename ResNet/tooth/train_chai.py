"""
Trains CHAI (the official model, formerly internally called "Comp3"): pooled (both-jaws-combined)
12-way classifier (this jaw's 2 quadrants x 6 digits - class12, same scheme as main_e12way.py
(moved to backups/superseded_20260831/)'s old E/Comp2 ablation) on the Mirrored Variant's
coordinate convention minus the X-mirror
(functions/features/main_chai.py's ResNet/tooth/csv_chai/features_{train,val,test}.csv).
Tests "how much does skipping the separate geometric-tens-assignment + Hungarian post-process
pipeline cost" - a CHAI prediction (class12) plus the already-known jaw (upper/lower)
...
the final answer, unlike the Mirrored Variant/Comp1/Comp2 which all still go through Hungarian.

Reuses ToothPositionClassifier's ResNet backbone/classifier-head shape from train.py via
train_mirrored.py's ToothPositionClassifierNoTheta (4-dim meta, no theta - just constructed with
num_classes=12 here instead of 6). Reuses train_mirrored.py's ToothDatasetCombined shape almost
...
Usage:
    .venv/Scripts/python.exe ResNet/tooth/train_chai.py [--resume]
"""
import sys
import os
import json
...
class ToothDatasetChai(Dataset):
    """Like train_mirrored.py's ToothDatasetCombined, but the label is class12 (0-11: this jaw's
...
def train_model(dataset_dir, train_csv_path, val_csv_path, model_save_path, runs_dir, epochs, batch_size, lr, patience=10, resume=False):
    ...
    train_dataset = ToothDatasetChai(csv_path=train_csv_path, dataset_dir=dataset_dir, split="train", transform=train_transform)
    print(f"Loading validation dataset from: {val_csv_path}")
    val_dataset = ToothDatasetChai(csv_path=val_csv_path, dataset_dir=dataset_dir, split="val", transform=val_transform)
    ...
    print("\nStarting ResNet18 Tooth Classifier (12 classes, no-mirror, pooled, CHAI) training...")
    ...
    plt.xlabel('Epoch'); plt.ylabel('Loss'); plt.title('Loss History (CHAI: Pooled No-Mirror 12-way)'); plt.legend()
    plt.subplot(1, 3, 2)
    plt.plot(epochs_range, train_accs, label='Train Acc', marker='o')
    plt.plot(epochs_range, val_accs, label='Val Acc', marker='o')
    plt.xlabel('Epoch'); plt.ylabel('Accuracy'); plt.title('Accuracy History (CHAI: Pooled No-Mirror 12-way)'); plt.legend()
    plt.subplot(1, 3, 3)
    plt.plot(epochs_range, train_f1s, label='Train F1', marker='o')
    plt.plot(epochs_range, val_f1s, label='Val F1', marker='o')
    plt.xlabel('Epoch'); plt.ylabel('F1-Score (Macro)'); plt.title('F1-Score History (CHAI: Pooled No-Mirror 12-way)'); plt.legend()
    plt.tight_layout()
    ...
    if Path(model_save_path).exists():
        ...
        plt.title("Confusion Matrix (Best CHAI Model)")
        plt.tight_layout()
...
def main():
    ...
    csv_dir = project_root / "ResNet" / "tooth" / "csv_chai"
    model_dir = project_root / "ResNet" / "tooth" / "model_chai"
    runs_dir = project_root / "ResNet" / "tooth" / "runs_chai"

    parser = argparse.ArgumentParser(description="Train CHAI: pooled, no-mirror, 12-way (up-to-tens-digit) tooth classifier - no Hungarian at eval time.")
