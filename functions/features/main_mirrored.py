"""
CSV builder for the Mirrored Variant (paper ablation label; formerly the main "Baseline"/"CHAI
(Ours)" model before CHAI was redefined to the unified 12-way, unmirrored architecture - see
main_chai.py, now the official model). Retained as a trained ablation checkpoint (Table
"Architecture-Component Ablation" in the paper) - do not delete or retrain over this without
updating that table.

Two defining properties vs. the retired production/variant-A scheme (main.py, moved to
backups/superseded_20260831/):

  1. No PCA coordinate rotation - x1/y1/x2/y2 come straight from the polygon's raw image-space
     bbox (coords_mirrored.get_normalized_coords_mirrored), mirrored/flipped by image geometry
     alone (image width/2 for the X mirror, image height for the Y flip - see that module's
     docstring). PCA (calculate_pca_rotation) is still computed here, but ONLY to feed
     compute_arch_order (X-sort row ordering by rotated-x) - a plain sort is robust to a missing
     tooth by construction, unlike the coordinate rotation and theta, both of which were shown to
     swing by ~13deg on average when a single posterior molar is missing. Note: CHAI's own
     pipeline (main_chai.py) no longer depends on this ordering at inference time at all.
...
Also combines both jaws into ONE CSV per split. The X mirror rule (tens in (2, 3)) plus a Y flip
for the whole lower jaw make all 4 quadrants land in one consistent coordinate convention for a
single pooled model (see coords_mirrored.py's docstring for the empirical derivation of both).

Writes to ResNet/tooth/csv_mirrored/.

Usage:
    .venv/Scripts/python.exe functions/features/main_mirrored.py [--force]
"""
import sys
import os
import csv
...
from coords_mirrored import get_normalized_coords_mirrored
    ...
    for json_path in json_files:
        ...
        try:
            ...
            for seg_data in sorted_segments:
                ...
                x1, y1, x2, y2 = get_normalized_coords_mirrored(
...
def main():
    ...
    output_dir = project_root / "ResNet" / "tooth" / "csv_mirrored"
    ...
    parser = argparse.ArgumentParser(
        description="Extract coordinates/FDI features with no PCA rotation, a unified (10s-"
                     ...
                     "split, 6-way digit label, no theta column - this is the Mirrored Variant "
                     "ablation."
    )
